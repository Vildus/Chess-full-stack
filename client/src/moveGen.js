/* eslint-disable */
import { calcMove, oponent } from "./chessClient.js"

export function moveGen(state) {
    ...
    moves = moves.filter((inpMove) => {
        let nestPos = calcMove(JSON.parse(JSON.stringify(state)), inpMove)
...
function getAttackMap(board, toMove) {
        ...
        let oponentChar = oponent(toMove)
        for (let y = 0; y< 8; y++) {
            for (let x = 0; x< 8; x++) {
